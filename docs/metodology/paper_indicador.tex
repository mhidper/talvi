...
Having established $r=1$, the second stage estimates the cointegrating vector $\vect{\beta}$ and the adjustment speed matrix $\vect{\alpha}$ simultaneously via restricted maximum likelihood. We normalize the cointegrating vector on GDP, so that the long-run equilibrium relationship can be written as
\[
\log y_t^{*} = \beta_0 + \beta_{\text{ToT}} \log ToT_t + \beta_{\text{rate}} US10Y_t,
\]
where $\log y_t^{*}$ denotes the equilibrium level of output implied by external fundamentals. 
Observed output can then be expressed as the sum of this equilibrium component and its deviation,
\[
\log y_t = \log y_t^{*} + ECT_t,
\]
where $ECT_t$ is the error-correction term capturing departures from the long-run equilibrium. 
The normalization on GDP is without loss of generality, since cointegrating vectors are identified only up to scalar multiplication. The adjustment speed vector $\vect{\alpha}$ then determines how each of the three variables responds to deviations from this equilibrium. Economic theory predicts that GDP should exhibit negative adjustment ($\alpha_y < 0$), meaning that when output exceeds its equilibrium level, subsequent growth is dampened as the economy corrects the overextension. In contrast, terms of trade and US yields are expected to be weakly exogenous ($\alpha_{ToT} \approx 0$, $\alpha_{rate} \approx 0$), since these external variables are determined in global markets and do not respond to Brazilian GDP deviations.
...
The choice of impulse response horizon $H$ for constructing the sensitivity vector $\vect{g}$ represents a trade-off between policy relevance (shorter horizons) and completeness of the dynamic response (longer horizons). We examine $H \in \{2, 4, 6, 8\}$ quarters, corresponding to six months through two years. The baseline choice of $H=4$ (one year) represents a middle ground appropriate for annual policy planning cycles. Vulnerability indicators constructed with alternative horizons correlate at $\rho > 0.89$ with the baseline, and the ranking of major episodes by severity is invariant to the horizon choice. Shorter horizons (e.g., $H=2$) place relatively greater weight on the near-term financial channel, as the GDP response to a U.S. 10-year yield shock materializes earlier than that to a terms-of-trade shock. This does not imply that the cumulative effect of the yield shock becomes more negative in absolute terms—indeed, the contemporaneous response is slightly positive and turns negative only from quarter~1 onward—but rather that, over two quarters, the share of total variance accounted for by the financial tightening channel is proportionally higher. At longer horizons (e.g., $H=8$), the slower yet more persistent effect of terms-of-trade shocks dominates the cumulative impact, while both configurations preserve the same episode classification and maintain a negative correlation with future growth\footnote{For reference, the cumulative loadings of the GDP response to each external shock are 
$g_{\mathrm{ToT}}(2) \approx 0.012$, $g_{\mathrm{ToT}}(4) \approx 0.021$, and 
$g_{\mathrm{ToT}}(8) \approx 0.037$ for terms-of-trade shocks; 
and $g_{\mathrm{US}}(2) \approx -0.005$, $g_{\mathrm{US}}(4) \approx -0.008$, 
and $g_{\mathrm{US}}(8) \approx -0.010$ for U.S. yield shocks. 
Hence, shorter horizons increase the relative contribution of the financial tightening channel even though its absolute cumulative effect remains smaller than that of the trade channel.}
.
...
To evaluate predictive accuracy, we convert the continuous vulnerability measure into a binary early-warning flag by marking quarters in which $V_t$ exceeds its 90th percentile. 
The resulting indicator attains a true positive rate (TPR) of $68\%$ and a false positive rate (FPR) of $22\%$, corresponding to an area under the receiver operating characteristic curve (AUROC) of 0.73. 
For a binary predictor such as this one, the ROC curve consists of two line segments and the AUROC reduces to the closed-form expression 
$\text{AUROC} = \tfrac{1}{2}(1 + \text{TPR} - \text{FPR})$, 
which equals 0.73 given the reported rates. 
This benchmark performs favorably relative to simpler binary rules based on individual indicators: a flag for when terms-of-trade growth falls below its 10th percentile yields AUROC~0.61, a flag for when U.S. yields exceed their 90th percentile yields 0.59, and a flag for when spreads exceed their 90th percentile yields 0.64.
