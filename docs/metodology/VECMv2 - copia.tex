...
\begin{abstract}
Este documento desarrolla y presenta un indicador econométrico de vulnerabilidad externa para grandes economía iberoamericanas mediante un Modelo de Vectores de Corrección de Errores (VECM) especialmente calibrado para capturar las particularidades estructurales de la economía brasileña. La metodología extiende el marco conceptual de Izquierdo, Romero y Talvi (2008) a través de cuatro innovaciones principales: especificación país-específica, construcción de un indicador sintético de vulnerabilidad, optimización endógena de pesos para maximizar capacidad predictiva, y evaluación sistemática de desempeño estadístico.

El modelo estima relaciones de cointegración entre el PIB brasileño y variables externas clave (términos de intercambio, producción del G7, condiciones financieras globales y flujos de capital), extrayendo el Término de Corrección de Error como base para un indicador compuesto que integra dos dimensiones: desequilibrios de largo plazo y volatilidad de corto plazo. La velocidad de corrección hacia el equilibrio se estima en 25\% trimestral, y mediante optimización matemática se determinan ponderaciones óptimas que mejoran la correlación predictiva en 24.4\% respecto a pesos equiponderados.

Los resultados empíricos, validados en el período 2000-2024 (99 observaciones), demuestran capacidad predictiva estadísticamente significativa para anticipar desaceleraciones económicas, con correlación negativa de -0.196 entre vulnerabilidad actual y crecimiento del PIB. El indicador identifica correctamente los principales episodios de estrés macroeconómico: crisis electoral 2002-2003, crisis financiera global 2008-2009, desaceleración estructural 2012-2015, y shock pandémico 2020-2021.

Este enfoque econométrico agregado se posiciona como complemento cuantitativo al sistema de monitoreo desagregado propuesto por Talvi et al. (2008), proporcionando una señal objetiva y sintética para alerta temprana que combina la sistematicidad del análisis formal con la riqueza interpretativa del análisis cualitativo, optimizando tanto la comprensión de fuentes específicas de vulnerabilidad como la eficiencia en la detección temprana de episodios de riesgo sistémico en economías latinoamericanas dependientes de commodities.
\end{abstract}
...
Las economías latinoamericanas, particularmente aquellas con elevada dependencia de la exportación de materias primas, enfrentan una exposición estructural a la volatilidad del entorno económico global. Las fluctuaciones en los precios de commodities, los cambios en las condiciones financieras internacionales, las variaciones en la demanda de los principales socios comerciales y los movimientos de los flujos de capital pueden generar shocks externos de magnitud considerable, con profundas repercusiones sobre el crecimiento económico, la estabilidad macroeconómica y la sostenibilidad fiscal. En este contexto, el desarrollo de herramientas robustas y oportunas para el monitoreo y la predicción de episodios de vulnerabilidad externa se constituye como una prioridad fundamental para la formulación de políticas macroeconómicas efectivas y la gestión proactiva del riesgo sistémico.

El objetivo principal de este trabajo es desarrollar y presentar un indicador econométrico de vulnerabilidad externa para economías iberoamericanas, implementado mediante un enfoque cuantitativo que integra tanto las relaciones de equilibrio de largo plazo como la dinámica de ajuste de corto plazo. A diferencia de los sistemas de monitoreo tradicionales que se basan en el seguimiento desagregado de múltiples variables a través de ``dashboards'' de indicadores, este estudio propone un enfoque sintético y agregado mediante la estimación de un Modelo de Corrección de Errores (ECM), posteriormente expandido a un indicador compuesto multidimensional.

La metodología implementada en este trabajo se basa en el Modelo de Vectores de Corrección de Errores (VECM), que permite capturar de manera efectiva las interrelaciones dinámicas entre las variables económicas relevantes. Este enfoque \textbf{se fundamenta en y extiende} el marco econométrico seminal propuesto por Izquierdo, Romero y Talvi (2008), aportando tres contribuciones metodológicas principales:

\textbf{1. De Análisis de Descomposición a Indicador Sintético Operacional:} Partiendo de la sólida base econométrica de Talvi para descomponer fluctuaciones del PIB en componentes externos versus internos, este trabajo avanza hacia la construcción de un indicador sintético compuesto que transforma los resultados del VECM en una herramienta operacional de monitoreo. El indicador combina dos dimensiones del riesgo macroeconómico: desequilibrios de largo plazo (ECT normalizado) y volatilidad de corto plazo, proporcionando una métrica única y temporalmente comparable.

\textbf{2. Incorporación de Optimización Endógena para Maximizar Capacidad Predictiva:} Mientras Talvi establece las relaciones econométricas fundamentales mediante funciones impulso-respuesta, este trabajo incorpora técnicas de optimización matemática para calibrar endógenamente las ponderaciones del indicador: $w^* = \arg \max_w |\text{Corr}(\text{IVE}_t(w), \Delta \text{PIB}_{t+h})|$. Esta extensión metodológica transforma el modelo de una herramienta de análisis retrospectivo a un sistema de alerta temprana cuantitativamente validado.

\textbf{3. Extensión hacia Capacidad Predictiva y Validación Estadística:} Complementando el enfoque retrospectivo y explicativo de Talvi, este trabajo desarrolla y valida rigurosamente la capacidad predictiva del indicador para anticipar episodios futuros de vulnerabilidad externa, utilizando métricas de desempeño estadístico, correlaciones predictivas con múltiples horizontes temporales, y técnicas de validación out-of-sample. Esta orientación prospectiva establece la credibilidad del indicador como herramienta operacional de política económica.

La implementación práctica de esta metodología abarca seis fases metodológicas secuenciales: (1) preparación y alineación de datos de múltiples fuentes (IBGE, OECD, FRED, Bloomberg), (2) análisis de propiedades estocásticas mediante tests de raíz unitaria, (3) estimación del modelo de corrección de errores con validación de supuestos, (4) diagnóstico integral del modelo y tests de robustez, (5) construcción del indicador compuesto de vulnerabilidad externa, y (6) optimización endógena de pesos mediante técnicas de programación secuencial cuadrática.

El modelo utiliza datos trimestrales para el período 2000-2024 para Brasil e incorpora cinco variables fundamentales: el PIB real (variable endógena principal), los términos de intercambio, la producción industrial del G7 (proxy de demanda externa), el rendimiento de bonos del Tesoro estadounidense a 10 años (condiciones financieras globales), y el spread de riesgo soberano (percepción específica de riesgo-país). Adicionalmente, se incluye una variable dummy para capturar los efectos atípicos de la pandemia de COVID-19.
...
\subsection{Especificación del Modelo para Brasil}

En este ejercicio, y siguiendo la metodología de Izquierdo, Romero y Talvi (2008), adaptamos el modelo de corrección de errores
(VECM) al caso particular de Brasil. A diferencia del trabajo original, que estima un VECM restringido para
un agregado regional (LAC7), aquí implementamos un \textbf{Modelo de Corrección de Errores (ECM)} con
integración mixta y estructura de rezagos optimizada.

\subsection{Variables}

Partiendo del marco conceptual de Izquierdo, Romero y Talvi (2008), mantenemos el mismo núcleo de factores externos —demanda internacional, términos de intercambio y condiciones financieras globales— pero trasladados al caso específico de Brasil. Esta continuidad garantiza comparabilidad con el estudio regional original, al tiempo que la adaptación nacional permite capturar la heterogeneidad estructural del país.

\begin{itemize}
    \item $ \log GDP_{BRA} $: logaritmo del PIB real de Brasil, variable endógena principal del modelo. \\
    \textit{Fuente: IBGE y OECD}.

    \item $ \log TOT_{BRA} $: logaritmo de los términos de intercambio de Brasil, que resume la influencia de los precios de commodities sobre el ingreso externo. \\
    \textit{Fuente: IBGE}.

    \item $ \log IP_{G7} $: índice ponderado de producción industrial de los países del G7 (logaritmos), como proxy de la demanda externa. \\
    \textit{Fuente: OECD}. 

    \item $ US10Y $: rendimiento de los bonos del Tesoro de EE.UU. a 10 años, indicador de liquidez y condiciones financieras globales. \\
    \textit{Fuente: Federal Reserve (FRED)}.

    \item $ Spread $: diferencial de riesgo soberano de Brasil frente a bonos del Tesoro de EE.UU., reflejo de la percepción de riesgo país. \\
    \textit{Fuente: Bloomberg}.
\end{itemize}

\subsection{Evaluación de la Existencia de Relaciones de Cointegración}

La posibilidad de que existan relaciones de cointegración se evalúa mediante pruebas de raíz unitaria (Augmented Dickey-Fuller, Phillips-Perron y KPSS). Así, una potencial combinación de series estacionarias y no estacionarias justificaría la estimación de un \textbf{Modelo de Corrección de Errores (ECM)} con cointegración, al que habría que añadir la necesaria evaluación del número de rezagos óptimos.

Se confirma la existencia de una relación de cointegración entre el PIB brasileño, los términos de intercambio
y la tasa de interés a 10 años de EE.UU. El término de corrección de error ($ECT_t$) se define como:

\[
ECT_t = \log GDP_{BRA,t} - \gamma_1 \log TOT_{BRA,t} - \gamma_2 US10Y_t
\]

\subsection{Especificación del ECM Optimizado}

La dinámica de corto plazo se modela mediante un ECM ampliado, con dos rezagos del $ECT$ y rezagos en las
variables externas, de acuerdo con los criterios AIC y BIC:

\[
\Delta \log GDP_{BRA,t} = \alpha_1 ECT_{t-1} + \alpha_2 ECT_{t-2} +
\beta_1 \Delta \log TOT_{BRA,t} + \beta_2 \Delta \log TOT_{BRA,t-1} +
\beta_3 \Delta US10Y_t + \beta_4 \Delta US10Y_{t-1} +
\delta_1 \log IP_{G7,t} + \delta_2 \log IP_{G7,t-1} +
\phi_1 Spread_t + \phi_2 Spread_{t-1} + \theta D_t + \varepsilon_t
\]

donde $D_t$ corresponde a la dummy de pandemia (2020Q1–2020Q3).

\subsection{Indicador de Vulnerabilidad Externa Optimizado}

El indicador compuesto se construye a partir de dos componentes esenciales, con pesos calibrados endógenamente:

\[
VI_t = 0.379 \times |ECT^{norm}_t| + 0.621 \times Vol^{norm}_t
\]

donde $ECT^{norm}_t$ es el término de corrección de error normalizado (z-score) y $Vol^{norm}_t$
es la volatilidad móvil de los residuos del modelo, normalizada en una ventana de 4 trimestres.

El sistema de alertas se define como:
\begin{itemize}
    \item Verde: $VI < 0$ (vulnerabilidad baja).
    \item Amarillo: $0 \leq VI < 0.5$ (vulnerabilidad moderada).
    \item Naranja: $0.5 \leq VI < 1.0$ (vulnerabilidad elevada).
    \item Rojo: $VI \geq 1.0$ (vulnerabilidad extrema).
\end{itemize}
...
La primera extensión consiste en la **especificación país-específica del modelo**. Mientras Talvi et al. analizan un índice agregado LAC7 que captura comportamientos promedio regionales, este trabajo desarrolla un modelo específicamente calibrado para cada país, con el ejemplo presente de Brasil, incorporando las particularidades estructurales de la economía de cada uno de ellos. Esta especificación permite capturar heterogeneidades específicas del país que se diluyen en el análisis regional agregado, como la importancia particular de ciertos commodities en la canasta exportadora o la sensibilidad específica a condiciones financieras globales dada la profundidad relativa del mercado de capitales.

La segunda extensión radica en la **construcción de un indicador sintético de vulnerabilidad externa**. Mientras el modelo de Talvi se enfoca en análisis de impulso-respuesta y ejercicios contrafactuales retrospectivos, este trabajo construye un indicador compuesto que combina dos dimensiones del riesgo macroeconómico: desequilibrios de largo plazo (ECT normalizado) y volatilidad de corto plazo. Esta síntesis permite generar una métrica única y temporal comparable de vulnerabilidad que facilita el monitoreo sistemático y la comparación intertemporal de episodios de riesgo.

La tercera extensión incorpora **optimización endógena de pesos** para maximizar la capacidad predictiva del indicador. A diferencia del análisis de Talvi, que utiliza funciones impulso-respuesta para estudiar efectos dinámicos históricos de shocks externos, este trabajo construye un indicador sintético compuesto que combina múltiples dimensiones de vulnerabilidad con ponderaciones optimizadas: $\mathbf{w}^* = \arg\max_{\mathbf{w}} |\text{Corr}(IVE_t(\mathbf{w}), \Delta PIB_{t+h})|$. Esta metodología representa una extensión fundamental que transforma el VECM de herramienta analítica a sistema de alerta temprana operacional.

La cuarta extensión consiste en la **evaluación sistemática de capacidad predictiva** mediante métricas de desempeño estadístico. Mientras Talvi et al. se concentran en explicar variaciones históricas del PIB mediante validación econométrica estándar (R², significancia, diagnósticos), este trabajo evalúa rigurosamente la capacidad del indicador para anticipar episodios futuros de contracción económica utilizando análisis ROC, correlaciones predictivas con múltiples horizontes temporales, y métricas de precisión en la clasificación de crisis. Esta validación cuantitativa es fundamental para establecer la credibilidad del indicador como herramienta operacional de política económica.
